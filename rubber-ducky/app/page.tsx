    ...
    <div className="flex flex-col h-screen overflow-hidden" style={{ background: 'var(--bg)', color: 'var(--text)' }}>
      ...
      >
        <div className="flex items-center gap-2.5">
          ...
          <span className="font-bold text-[15px] tracking-tight">DuckPad</span>
        ...
        <div className="flex items-center gap-3 text-xs" style={{ color: 'var(--muted)' }}>
          <span>Device: <span className="font-mono font-semibold" style={{ color: 'var(--text)' }}>{deviceId || '—'}</span></span>
          <span className={`font-semibold ${statusColor[deployStatus]}`}>
            {deployStatus === 'sent' && (
              <span className="inline-block w-1.5 h-1.5 rounded-full mr-1.5 align-middle dot-polling"
                    style={{ background: 'var(--warn)' }} />
            )}
            {deployStatus === 'fetched' && (
              <span className="inline-block w-1.5 h-1.5 rounded-full mr-1.5 align-middle dot-connected"
      ...
      {/* ── 3-column layout ────────────────────────────────── */}
      <div className="flex flex-1 overflow-hidden" style={{ height: 'calc(100vh - 50px)' }}>

        {/* ── LEFT SIDEBAR ─────────────────────────────────── */}
        <aside
          className="flex flex-col shrink-0 overflow-y-auto border-r"
          style={{ width: 238, background: 'var(--bg2)', borderColor: 'var(--border)' }}
        >
          {/* OS selector */}
          <div className="p-3.5 pb-2">
            ...
            <div className="grid grid-cols-2 gap-1.5">
              {OS_OPTIONS.map(os => (
                <button
                  key={os.id}
                  onClick={() => { setSelectedOS(os.id); setActivePayloadId(null) }}
                  className="rounded-lg py-2 px-1.5 flex flex-col items-center transition-all text-center"
                  style={{
          ...
          {/* Payload list */}
          <div className="p-3.5 pt-2 flex-1">
          ...
          {/* New custom payload */}
          <div className="p-3.5 pt-0 border-t" style={{ borderColor: 'var(--border)' }}>
            <p className="text-[10px] font-semibold uppercase tracking-widest mb-2" style={{ color: 'var(--muted)' }}>
              Custom
            </p>
            <button
        ...
        <main className="flex flex-col flex-1 min-w-0 overflow-hidden">
          ...
          >
            ...
            <button
              ...
              className="flex items-center gap-1.5 px-3 py-1.5 rounded-md text-[12px] font-semibold transition-all disabled:opacity-40 disabled:cursor-not-allowed"
          ...
          <div className="flex flex-1 overflow-hidden relative" style={{ background: 'var(--bg)' }}>
            ...
            <div
              ref={lineNumRef}
              className="shrink-0 overflow-hidden select-none text-right font-mono text-[12.5px] leading-relaxed pt-4 pb-4 pr-2 pl-4 border-r whitespace-pre"
              style={{
                minWidth: 44,
                color: 'var(--muted)',
                background: 'var(--bg2)',
            ...
            <textarea
              ...
              spellCheck={false}
              className="flex-1 resize-none outline-none font-mono text-[12.5px] leading-relaxed px-4 pt-4 pb-4 whitespace-pre overflow-auto"
              style={{
                background: 'var(--bg)',
                color: 'var(--text)',
                lineHeight: '1.625rem',
        ...
        <aside
          ...
          style={{ width: 300, background: 'var(--bg2)', borderColor: 'var(--border)' }}
        >
          {/* Tabs */}
          ...
          {/* ── DEPLOY TAB ─────────────────────────────────── */}
          {activeTab === 'connect' && (
            <div className="flex flex-col flex-1 overflow-y-auto p-3.5 gap-3">

              {/* Device ID card */}
              <div className="rounded-lg p-3.5" style={{ background: 'var(--bg3)', border: '1px solid var(--border)' }}>
                <p className="text-[10px] font-semibold uppercase tracking-widest mb-2.5" style={{ color: 'var(--muted)' }}>
                  Device ID
                </p>
          ...
          {/* ── REFERENCE TAB ──────────────────────────────── */}
          {activeTab === 'reference' && (
            <div className="flex-1 overflow-y-auto p-3.5 space-y-4 text-[11px] font-mono">
              <div>
